var API = 'https://nyaaapi.herokuapp.com/nyaa/anime?query='

function insertAfter(referenceNode, newNode) {
    referenceNode.parentNode.insertBefore(newNode, referenceNode.nextSibling);
}

var animeTitle = document.getElementsByTagName('h3')[0].innerText
var summaryElement = document.getElementsByClassName('media-summary')[0]
var torrentElement = document.createElement('div')
    ...
    fetch(API + animeTitle)
        .then((response) => {
            ...
            response.json().then((res) => {
                ...
                inflator(res.data)
                insertAfter(summaryElement, torrentElement)
            })
        })
} catch (err) {
    console.log(err)
